...
[`PostgresProviderTests`](Collega.Infrastructure.Tests/PostgresProviderTests.cs) is the **only** test in the repo that talks to a real database, and the **first and only one that is not hermetic**. It runs a `postgres:16` container via Testcontainers. The nearest precedent, [`SkiaSharpImageProcessorTests`](Collega.Infrastructure.Tests/SkiaSharpImageProcessorTests.cs), is a precedent only for the *shape* of the argument — reach for the real component when a fake could not prove the thing, and say why in the class itself. That test is still hermetic; this one genuinely is not.
