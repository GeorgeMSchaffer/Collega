...
- **`--seed:auth=reset` (dev/ops affordance, not product behavior):** drops the existing Site Admin matching the configured `SiteAdmin:Email` and recreates it from `SiteAdmin:Email` / `SiteAdmin:Password` with `MustChangePassword: true` — use it to recover a forgotten/rotated Site Admin password locally. It forces the Site Admin seed on (implies `--seed:auth`) but does **not** seed demo data, and it targets only the configured account, so a manually promoted Site Admin on a different email is left alone. Typical use: `dotnet run --project ./src/Collega.API -- --seed:auth=reset`. Also accepts `/seed:auth=reset`.
