...
**Comp C "Fluent Editorial"** — `SPEC/mockups/comp-c-fluent-editorial.html`, specified in `SPEC/20-feature-client-ui.md`: slim 64px icon rail, serif display headings, cool neutral palette with a desaturated blue accent (**changed 2026-08-31**, see `SPEC/20-feature-client-ui.md` → Color palette; `app.css` is not yet migrated), page-header tabs, list-style status sections (grouped rows, not Kanban columns) for boards, and — **since 2026-08-10** — an idea detail that opens as a **right slide-in drawer** (detail + inline edit) with a centered **create modal**, replacing the earlier full-page idea detail. See `SPEC/20-feature-client-ui.md` → Idea Detail Surface and `SPEC/mockups/comp-c-review-09-detail-surfaces.html`. Built (T-UI-3) in `Components/IdeaDrawer.razor` + `Components/IdeaCreateModal.razor`, used from the Ideas list and board rows/cards.
...
**Colour has meaning — don't reuse the accent for a new one.** The accent (`--accent`) means *active / selected / primary action* everywhere in Comp C. It was indigo `#5b5fc7`; **since 2026-08-31 the specified accent is the desaturated blue `#527292`** (fill `#5B7FA3`). The *rule* is unchanged and survives the recolour — what the accent means did not move, only its hue. `app.css` still ships indigo; see `SPEC/20-feature-client-ui.md` → Color palette for the target and the outstanding migration. When Sprint 7's AI assist needed to mark "the assistant proposed this value", it got its **own teal token** rather than borrowing indigo, because a suggestion is neither active nor selected and overloading the accent would make both meanings unreadable. Locked as D-SUGGEST in `SPEC/20-feature-ai-idea-assist.md`. Apply the same test before introducing any further state colour. **The 2026-08-31 recolour narrows this gap** — indigo sat ~68° of hue from `--sug: #116b5e`, the new blue sits ~38° away — so when `app.css` is migrated, verify the suggestion chip, tinted field and left border still read as clearly not-the-accent.
