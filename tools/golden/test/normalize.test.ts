...
test('a guid is labelled by where it was first seen, and keeps that label everywhere', () => {
  ...
  assert.equal(
    n.string(BOARD.toUpperCase(), 'x'),
    '<guid@create.body.id>',
    'case is not an identity difference',
  )
...
test('identity relationships survive normalization inside one response', () => {
  ...
  const normalized = n.value(
    { id: BOARD, boardId: BOARD, organizationId: OTHER },
    'body',
...
test('a password the API mints is redacted too, not just one we sent', () => {
  ...
  const issued = redact({ temporaryPassword: 'Tmp-4821!', mustChangePassword: true }) as Record<
    string,
    unknown
  >
  ...
  const imported = redact({
    rows: [
      {
        rowNumber: 1,
        email: 'new@demo.collega.test',
        outcome: 'Created',
        temporaryPassword: 'Tmp-9930!',
      },
      {
        rowNumber: 2,
        email: 'bad',
...
test('credentials never reach a fixture, at any depth', () => {
  ...
  assert.equal(
    redacted.email,
    'orgadmin@demo.collega.test',
    'the identity is the point of the case',
  )
...
test('object keys are sorted, so serialization order is not a finding', () => {
  const n = new Normalizer()
  assert.deepEqual(Object.keys(n.value({ b: 1, a: 2, c: 3 }) as object), ['a', 'b', 'c'])
})
...
test('diff reports missing and extra keys separately', () => {
  ...
  assert.deepEqual(mismatches.map((m) => [m.path, m.kind]).sort(), [
    ['body.b', 'missing'],
    ['body.c', 'extra'],
  ])
})
