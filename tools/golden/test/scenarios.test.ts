...
const ok = {
  name: 'boards',
  steps: [
    { id: 'list', endpoint: 'GET /boards/{boardId}', as: 'OrgAdmin', kind: 'success', expect: 200 },
  ],
}
...
test('validation rejects the mistakes a hand-written corpus actually makes', () => {
  const cases: [string, unknown][] = [
    ...
    [
      'missing endpoint',
      { name: 'x', steps: [{ id: 'a', as: 'User', kind: 'success', expect: 200 }] },
    ],
    ...
    [
      'no expect',
      { name: 'x', steps: [{ id: 'a', endpoint: 'GET /x', as: 'User', kind: 'success' }] },
    ],
...
test('pluck collects a field from every element, which the reorder endpoints need', () => {
  const source = {
    body: [
      { statusId: 's1', name: 'New' },
      { statusId: 's2', name: 'Done' },
    ],
    headers: {},
  }
...
test('the upload and credential fields are checked like everything else', () => {
  ...
  const cases: [string, unknown][] = [
    [
      ...
      { ...base, steps: [{ ...ok.steps[0], file: { field: 'csvFile' } }] },
    ],
    [
      'credentials without a password',
      { ...base, steps: [{ ...ok.steps[0], credentials: { email: 'a@b' } }] },
    ],
    [
      'a file and a body together',
      {
        ...base,
        steps: [
          {
            ...ok.steps[0],
            body: {},
            file: { field: 'f', filename: 'f.csv', contentType: 'text/csv', content: 'a' },
          },
        ],
      },
    ],
