...
test('routes combine the class prefix with the method template, constraints stripped', async () => {
  ...
  assert.deepEqual(
    ...
    ['POST /auth/login', 'GET /auth/me', 'GET /organizations/{organizationId}/ai-assist/usage'],
  )
...
test('two actions claiming one route is an error, not a silent overwrite', async () => {
  const clash = SAMPLE.replace('GetOrganizationUsage', 'Login')
    .replace('[Route("organizations/{organizationId:guid}/ai-assist")]', '[Route("auth")]')
