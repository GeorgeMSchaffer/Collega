...
import { type Exchange, Runner } from '../src/runner.ts'
import { type Scenario, validateScenario } from '../src/scenarios.ts'
import { STUB_CREDENTIALS, type StubOptions, startStub } from './stub-api.ts'
    ...
    steps: [
      {
        ...
        kind: 'success',
        expect: 200,
        body: STUB_CREDENTIALS.OrgAdmin,
        unstable: ['body.expiresInSeconds'],
      },
      {
        id: 'create',
        endpoint: 'POST /organizations/{organizationId}/boards',
        as: 'OrgAdmin',
        kind: 'success',
        expect: 201,
        path: '/organizations/{{orgId}}/boards',
        body: { name: 'Ideas' },
        bind: { boardId: '$.body.id' },
      },
      {
        id: 'read',
...
test('a capture replayed against the same stack matches, despite fresh guids and tokens', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      report.total,
      JSON.stringify(
        report.results.filter((r) => r.status !== 'match'),
        null,
        2,
      ),
    )
...
test('a changed response body is caught, and named by path', async () => {
  ...
  try {
    ...
    const drifted = await runAgainst({
      mutate: (route, body) =>
        route.endsWith('/boards') && 'name' in body
          ? { ...body, title: body.name, name: undefined }
          : body,
    })
...
test('no password reaches a fixture', async () => {
  ...
  try {
    ...
    assert.ok(login, 'the corpus has a login step whose redaction can be checked')
    const loginBody = login.request.body as Record<string, unknown>
    assert.equal(loginBody.password, '<redacted>')
    assert.equal(
      loginBody.email,
...
test('a changed status is reported on its own, not buried under body noise', async () => {
  ...
  try {
    ...
    assert.deepEqual([failure?.mismatches[0].expected, failure?.mismatches[0].actual], [401, 403])
  } finally {
    await rm(dir, { recursive: true, force: true })
  }
})
