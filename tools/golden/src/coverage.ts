...
import { type Endpoint, ROLES, type Role } from './inventory.ts'
...
export function report(
  ...
  for (const endpoint of endpoints) {
    ...
    for (const [role, roleKinds] of roles) {
      cells.push({
        ...
        role: role as Role | 'anonymous',
        kinds: [...roleKinds].sort(),
      })
  ...
  return {
    ...
    successOnly,
    cells: cells.sort(
      (a, b) => a.endpoint.localeCompare(b.endpoint) || a.role.localeCompare(b.role),
    ),
  }
}
...
export function formatReport(r: CoverageReport): string {
  ...
  if (r.partialRoles.length > 0) {
    ...
    for (const { endpoint, missing } of r.partialRoles)
      lines.push(`  ${endpoint} — ${missing.join(', ')}`)
  }
