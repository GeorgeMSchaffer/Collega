...
const commands: Record<string, (args: Args) => Promise<number>> = {
  async inventory(args) {
    ...
    for (const e of list) {
      const auth =
        e.authorize === 'anonymous' || e.authorize === 'any' ? e.authorize : e.authorize.join('+')
  ...
  async scaffold(args) {
    const { list } = await endpointMap()
    const written = await scaffoldScenarios(
      list,
      PATHS.scenarios,
      args.flags.get('force') === 'true',
    )
    console.log(
      `scaffolded ${written.length} scenario file(s) into ${path.relative(REPO, PATHS.scenarios)}`,
    )
  ...
  async coverage(args) {
    ...
    const cases =
      source === 'fixtures'
        ? (await readCorpus(PATHS.fixtures)).map((f) => ({
            endpoint: f.endpoint,
            role: f.as,
            kind: f.kind,
          }))
