...
- **The E2E suite exhausts the auth rate limiter and fails for a reason unrelated to the product.** Found 2026-09-14 while recording the suite. It performs **27 sign-ins per full run** across five spec files (`AUTH_THROTTLERS`, `apps/api/src/auth/rate-limit.guard.ts`). **Corrected 2026-09-14: this named the wrong bucket and was far too generous.** The binding limit is login's **twenty per minute**, not the hundred per hour, and a *single* full run exceeds it — 27 sign-ins inside a run that takes under two minutes. It was first seen as "the fourth consecutive run" because until the isolation fix below, nobody had run the whole suite at once. Consecutive runs then exhaust the hourly hundred as well, so both buckets are in play. Confirmed by experiment: `signs-in.spec.ts` fails in a full run and passes in isolation, on the same commit and the same data. **This is the same collision `tools/golden`'s replay hit** — the entry above it is the same problem in a different harness — so a fix should probably serve both. The candidates and their costs are argued in `SPEC/decisions.md` 2026-09-12; the one that suits a browser suite best and is not listed there is Playwright's `storageState`: sign in once per role and reuse the session, which would take 27 sign-ins to about four. The journey spec must keep signing in for real, because that is what it exists to test.

  **Half-fixed 2026-09-14.** The three copied sign-in helpers are now one (`e2e/tests/sign-in.ts`), and it tells a rate-limited refusal from a wrong credential — which the screen does not: `LoginForm` renders the same sentence for both and differs only by a trailing "Five failed attempts" that is deliberately absent when the limiter turned the request away. So the failure now says what it is instead of reporting a bare navigation timeout that reads as a bad password. **The suite is still not repeatable**; `storageState` remains the fix.

- **The E2E suite ran against the developer's own database, not its isolated schema.** Found and fixed 2026-09-14. `playwright.config.ts`'s `webServer` env is an object literal evaluated at module load, which is before `globalSetup` runs, so `DATABASE_URL` was read from a variable global setup had not written yet. The API received an empty string and its config fragment fell back to the `POSTGRES_*` parts in `.env` — `public`, with no `?schema=`. Global setup dropped, rebuilt and seeded `collega_e2e` on every run and nothing touched it; fourteen `Journey Co` organizations had accumulated in the local development database. The derivation now lives in `e2e/database-url.ts` and both files import it. Two consequences worth keeping: every isolation claim in `global-setup.ts` was true of a schema no test reached, and the defect hid a second one, because debugging a failing write meant querying the wrong schema.

- **The board page renders two level-1 headings.** Found 2026-09-14 while writing `e2e/tests/demo-path.spec.ts`, which could not address the heading without `.first()`. The topbar breadcrumb renders `<h1 class="text-base font-semibold tracking-tight">Boards / <name></h1>` and the page body renders `<h1>{board.name}</h1>` below it. A document should have one; a screen reader user hears the page announce two different top-level titles, and the breadcrumb is not a title. **Checked 2026-09-14, and it is systemic: eleven screens render two.** `Topbar` hardcodes `<h1>` and is used by eighteen screens; eleven of those render an `<h1>` of their own underneath it — both board pages, all five delivery screens, the ideas list, settings, profile, and everything through `SettingsPage` or `RefusalPanel`.

  The fix is not one line, which is why it is recorded rather than done. `Topbar`'s title is a frame and a breadcrumb, not a document title, so it should stop being a heading — but **six screens rely on it as their only `<h1>`** (the boards list, home, an idea's detail, the design-system page, and the desk-level `error`/`loading`), and demoting it would leave those with no top-level heading at all. Doing it properly means demoting `Topbar` and giving those six a heading of their own, across roughly seventeen files. The spec suite works either way, so this is still not urgent and not blocking anything.
