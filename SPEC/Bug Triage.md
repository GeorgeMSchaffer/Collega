...
### T-UI-3 · Idea Detail slide-in drawer + create modal — **BUILT + MERGED to `dev` (2026-08-10); pending live browser verification**
Delivered via a multi-agent worktree pass (Wave 1: drawer/modal foundation + Ideas integration, and T-UI-2 admin-list toolbars; Wave 2: board integration; plus the T-UI-1 icon fold-in and a code-review pass). Full-page `IdeaDetail.razor` (`/ideas/{id}/edit`) retired; `?idea={id}` on the current route + bare `/ideas/{id}` opens the Ideas list with the drawer. Full parity in `Components/IdeaDrawer.razor` (fields + UDFs, tags, mentions, 0–5 assignees, comments, upvote, status move with live re-slot, admin delete) + `Components/IdeaCreateModal.razor`, used from the Ideas list and board rows/cards.
- **Validation so far:** clean client + full-solution builds; full **474-test** suite green; an independent code-review pass (5 findings fixed — cross-idea comment-draft leak, interleaved-load guard, Site-Admin create dead-end, nullable `OnCreated`, upvote aria-label).
- **Remaining before COMPLETED:** a live browser verification of the running app (drawer open/edit/save/status-move/comment/upvote/delete + create modal, from every entry point). Only then move this item to COMPLETED.
