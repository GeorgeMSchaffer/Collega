...
* **[Likely stale-build artifacts — needs user re-verify against a fresh build, 2026-08-11]** Three reported bugs describe pre-drawer behavior that the current `dev` code no longer exhibits:
    ...
    * *"Manage button on the Idea List Items does nothing"* — the `/ideas` list row action is **Details** (opens `IdeaDrawer`); there is no "Manage" button on that list. (Site Admin cross-org admin lists do have "Manage" links — that specific bug was real and is fixed.)
