...
**Explicitly out of scope for these sprints** (confirmed with the user): the in-progress .NET E2E work, which was discarded with the rest of that stack in slice F6 and never scheduled here. Also out of scope: anything post-MVP (OAuth, SAML, self-service password reset, Org AI credentials, the Roadmaps/Sprints/Tasks brainstorm in `Bug Triage.md`'s `## IDEAS` section) — none of that is "pre-MVP."
...
| ~~8~~ | — | **Cancelled 2026-09-04.** Its dependency chain is moot: nothing is deployed to Azure. The gate it carried — Sprint 5's Postgres migration verified against a real instance — is satisfied and now belongs to Wave F, which deploys to Vercel instead. |
| 9 | 7.5 | The conversion. Wave 0 was gated on conversion ticket `08`, decided 2026-09-04, so nothing outside this repo's own sprint order blocks it now. |
