...
- UI/UX Developer: 4 Comp C-style HTML review comps for design sign-off before any Blazor work starts, per the pivot to Comp C. See `SPEC/mockups/comp-c-review-01-login-and-auth.html`, `-02-admin-orgs-users.html`, `-03-board-list.html`, `-04-idea-detail.html`. None locked yet — all four open for review. Note the board pattern changed from Kanban columns (Comp A) to grouped list-sections by status (Comp C's "lanesec" pattern) and idea detail changed from a two-pane overlay (Comp A) to a full-page article layout (Comp C) — both still represent the same underlying swimlane/idea-detail behavior from the specs, just a different visual metaphor.
