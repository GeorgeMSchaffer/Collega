...
- Canonical spec updates still open pending user confirmation: Status's new Color and Sort Order fields, the proposed default status set (New, Research In Process, In Review, Complete), and the last-status-minimum vs. board's 2-swimlane-minimum interaction — none of this is in `SPEC/20-feature-boards-and-statuses.md` yet, only in the review comps.
