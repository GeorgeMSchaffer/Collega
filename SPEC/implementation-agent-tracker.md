...
## Current Status
**Verified 2026-09-03 against `830b136` (`dev`, which now carries the comp P refresh, comp Q, the conversion map, the Wave A harness and its first captured corpus; `main` behind, pending its PR).** Keep this section a table plus short blocks — see Maintenance Rule at the end of this file.
...
| Wave A (golden tests) | **A1 + A3 built; A2 started — 6 of 81 endpoints** | `tools/golden/` (2026-09-03) — capture and replay, zero-dependency TypeScript, 43 self-tests, `tools/golden/README.md`. The auth corpus is captured from the live API and replays 16/16 clean against a fresh seed. **The remaining 75 endpoints are A2's work, and it must finish before Sprint 8 retires the stack** — after that the conversion has no oracle. |
