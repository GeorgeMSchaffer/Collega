...
## Current Status
**Verified 2026-09-03 against `dbc47b5` (`dev`, which now carries the comp P refresh, comp Q, the conversion map and all of Wave A; `main` behind, pending its PR).** Keep this section a table plus short blocks — see Maintenance Rule at the end of this file.
...
| Test suite | **816 green, 0 failed** (2026-09-03 at `dbc47b5`) | 142 Domain + 324 Application + 148 Infrastructure + 202 API. **16 skipped, and the skips matter:** 5 Infrastructure `PostgresProviderTests` skip without Docker (they ran and passed on 2026-08-17, so a green run here does *not* mean the Postgres provider was exercised — start the container to cover it), and 11 `Collega.E2E.Tests` skip without a running app. Re-run before trusting. |
...
| Conversion map | **Ported; 5 of 12 tickets open** | `SPEC/typescript-conversion-map/` (from `feature/068`, 2026-09-03). Answered 2026-09-03 in `decisions.md`: `01` (comp P on Tailwind + shadcn/ui; Q C — Loop, decision records, commitment strip, Triage Mode in as **Wave G**, after F1), `02` (Vercel + Prisma Postgres; Nest is serverless, so no in-process state), `10` (discard the .NET suite; golden corpus + per-slice Vitest). Still open: `05` and `07` (AFK research, not started), `06` and `08` behind them, `11`. **`08` gates Wave 0.** `feature/068` also holds `src/prisma.md` with a credential — never merge it; rotate the key. |
