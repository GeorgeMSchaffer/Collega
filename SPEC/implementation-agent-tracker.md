...
**The transcript-cap conflict is resolved (user decision, 2026-08-17): `30-Contracts.md`'s "max 20 entries" wins.** Rule 5 had said "20 user turns"; it now counts entries, which halves the conversation to **10 user turns** since a transcript alternates and must end with a user entry. `IdeaAssistService.MaxUserTurns` became `MaxTranscriptEntries`, the redundant user-turn check is gone, and `turnsRemaining` is derived from the transcript length as it will stand *after* the turn — one longer for an accepted turn, one shorter for a refused one, since rule 8 drops it.
