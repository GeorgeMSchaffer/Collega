...
| Config / env schema | Every feature adds keys | Foundation defines a typed config module that composes **per-feature schema fragments**. Features add a fragment file; nobody edits the root schema. **Built as S0.4** in `apps/api/src/common/config/` — `.env` is the single home, `ANTHROPIC_API_KEY` is the only name for the AI key (`decisions.md` 2026-09-06). |
