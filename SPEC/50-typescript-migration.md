...
| **S0.2** Prisma introspect + reshape | `packages/infrastructure/prisma/**` — `db pull`, then the deliberate reshape; generated client; per-feature seed composition — including the **demo seed** that populates the platform for a walkthrough (`SPEC/10-requirements.md`), whose administrator credentials come from `DEMO_SEED_USERNAME`/`DEMO_SEED_PASSWORD` and never from source. Also owes the **three partial unique indexes** introspection drops, as raw SQL plus a test that fails if any is absent (`05`'s findings). **Freezes the schema.** |
