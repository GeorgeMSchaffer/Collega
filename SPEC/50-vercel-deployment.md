...
database; step 10 is verification; steps 11–12 populate the deployment and get people into it.
...
11. **Populate the deployment** — once, from a shell with the production `DATABASE_URL` exported,
    **not** from the build command and **not** as project environment variables (§8a says why):

    ```
    BOOTSTRAP_ORG_TITLE='<the organization>' \
    BOOTSTRAP_ORG_DESCRIPTION='<one line about it>' \
    BOOTSTRAP_ORG_ADMIN_EMAIL='<the first Org Admin>' \
      pnpm --filter @collega/infrastructure db:bootstrap-organization
    ```

    It prints an Org Admin temporary password (only if you set the third variable) and the
