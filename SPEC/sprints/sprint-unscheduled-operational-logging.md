> **Unscheduled, and its premises have changed (re-filed 2026-09-08).** Written 2026-08-17 as
> "Sprint 7.5", a slot that now belongs to `sprint-07.5-accessibility-and-bug-paydown.md`; two
> files cannot both be 7.5, so this one is renamed rather than renumbered into a real slot. It also
> targets the .NET stack, Serilog and an Azure App Service — all three superseded: the stack is
> frozen (`SPEC/decisions.md` 2026-09-06), Sprint 8 was cancelled 2026-09-04, and deployment goes
> to Vercel. **The gap it names is still real and still unowned** — the exception handler logs
> nothing, so a 500 leaves no trace — and it carries over to Nest rather than being fixed here.
> Kept for the problem statement and the redaction test list, not the implementation.
