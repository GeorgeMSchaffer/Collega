...
| P1 | Profile portrait upload: GIF/JPEG/PNG only, validated against malicious content, resized client- or server-side to 25×25px, replaces the initials avatar when set | Independent of the review pass — can run in parallel | None |
...
- [ ] Profile portrait upload built, validated, resized, and tested (including a rejection test for disguised non-image content)
