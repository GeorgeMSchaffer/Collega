...
> **Outcome (2026-08-11):** Delivered and merged to `dev` (backend + client slices). The audit found most server-side plumbing already existed (`search`, `sortBy`, `sortDirection`, `scope`, field filters; `tag` on the board endpoint). Gaps closed on the **global org ideas endpoint**: all-column `search` (Title, Created By, Assigned To, Status, and Created Date via ISO term), a `tag` filter, a new `user` filter (ideas the user authored **or** is assigned to), and five-key server-side `sortBy` (createdAt/title/createdBy/assignedTo/status) with an idea-id tiebreaker for stable pagination. Client: sortable `/ideas` column headers (arrow + `aria-sort`), a tag filter input, and a "filter by user (created or assigned)" picker — all reset to page 1 and compose (AND) with the existing scope chips / type-status dropdowns / field filters. Contract + feature spec updated (`SPEC/30-Contracts.md`, `SPEC/20-feature-ideas-and-engagement.md`). **+20 tests (498 green); build 0/0.** SQL-translatability of the correlated-subquery sorts/filters — which the InMemory tests can't prove — was **live-verified against real SQL Server** (every param returns 200; e.g. `tag=automation`→2), and a live UI QA confirmed the header sort (title-ascending) and tag filter (→2 rows) round-trip. Admin list pages' existing `ListToolbar` client-side all-column search was audited and left as-is (adequate for the small fully-loaded lists). Archived at `SPEC/sprints/archive/sprint-03-list-filter-parity.md`.
