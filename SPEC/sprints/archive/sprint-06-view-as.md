...
**Status:** **Complete (2026-08-14).** Every Definition of Done item built and reviewed; 622 tests green at `a0ef22c`, which is now the tip of both `dev` and `main`. The last P1, the Site Admin direct org-content mutation retirement, closed the same day: enforcement moved server-side into the Application layer, so it no longer depends on which route the client happened to render. A review pass over that change found two unguarded mutation paths (`ReassignIdeaTypeAsync`, `ImportBoardIdeasAsync`) — both fixed, covered, and mutation-checked.

**Carried out of this sprint deliberately:** the `ViewAsAuth` test-harness leak, moved to `sprints/archive/sprint-06.5-bug-fixes-and-tweaks.md` as scope item 1, where it was fixed. It is a test-quality issue, not a product defect, and its blast radius (5 test classes, 82 call sites) did not belong in an already-large diff.

**Note on the `main` merge:** `main` had been left at `1973592 "WIP"` and was a strict ancestor of `dev`, 270 commits behind. The epic exit criteria's "PR to `main`" was therefore not a Sprint 6 review — it was a branch reconciliation, so `main` was fast-forwarded to `dev` (user decision, 2026-08-14). Keep `main` current per epic from here so the next one is a genuinely reviewable PR.
...
- [x] Client UI built — drawer picker, active banner, rail swap, exit; mutations work under act-as — `Components/ViewAsDrawer.razor`, `Components/ViewAsBanner.razor`, `Layout/NavRail.razor`, `Layout/MainLayout.razor`
  - **Correction (2026-08-14): this originally read "matches the locked comp (control, …)" and that was wrong.** D-PLACE locked **two** entry points; only the rail avatar-menu item was built, so the page-header control is missing and the feature is reachable only through a popover at the bottom of the rail. The user could not find it on `dev` at all. The line was checked off without verifying it against the comp, and this sprint was marked Complete on that basis. Carried to `sprint-06.5-bug-fixes-and-tweaks.md` scope item 2 rather than reopening this sprint.
