...
import { ACCOUNTS, fillFluent, loginExpectingShell, selectFluent } from './helpers'
...
test.describe
  .serial('Org Admin user management', () => {
    ...
    test('deactivates the two users', async ({ page }) => {
      ...
      for (const u of USERS) {
        ...
        await page
          .getByRole('row', { name: new RegExp(u.email) })
        ...
        await expect(page).toHaveURL(/\/settings$/)
        await expect(
          page.getByRole('row', { name: new RegExp(u.email) }).getByText('Inactive'),
        ).toBeVisible()
      }
    })
  })
