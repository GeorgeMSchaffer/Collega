...
import { ACCOUNTS, fillFluent, loginExpectingShell } from './helpers'
...
test.describe
  .serial('Board, card, and status moves', () => {
    ...
    test('User moves the card through all statuses', async ({ page }) => {
      ...
      const optionLabels = (await statusSelect.locator('option').allTextContents()).map((s) =>
        s.trim(),
      )
