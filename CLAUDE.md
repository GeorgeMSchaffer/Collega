...
**Still open:** a dedicated `/ideas` page (global idea search, distinct from a board's own view) has no comp yet — the rail icon now points at it, but the page itself needs its own design pass before Client build reaches it. A mobile/narrow-viewport pass for the icon rail and Idea Detail's fixed two-column layout is also undesigned.
