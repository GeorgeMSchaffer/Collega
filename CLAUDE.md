...
**Locked (2026-08-07):** `SPEC/mockups/comp-c-review-06-lockin-v4-combined.html` is the chosen direction for Sign in, Home, Admin (Orgs/Users lists), Board List, and Idea Detail — it supersedes `comp-c-review-01`, `-02`, `-04`, and `-05` for general chrome/color/spacing (those four remain the reference for the detailed CRUD states v4 doesn't repeat: new/edit/detail/CSV-import forms, status color picker, all sign-in edge-case screens). Key decisions baked into v4: the 64px icon rail stays (see the open header-bar question below), the richer Home dashboard from Decision D4 in `SPEC/20-feature-client-ui-revisions.md`, minimal border radius throughout, and Board List rows/headers colored to match each status (tying List and Swim Lanes to one visual system).

**Resolved (2026-08-07, same session):** Swimlane card treatment is **Flat** (pale lane background, priority-colored left border per card, small status dot in the lane header) — the Banded and Tinted variants were considered and dropped. The `rgb(33,37,41)` header bar is dropped entirely; the rail's bottom avatar now owns Sign Out/Profile access. Both are reflected in `SPEC/20-feature-client-ui.md` and `SPEC/20-feature-client-ui-revisions.md`.

**Still open:** the rail comps never included a dedicated `Ideas` icon, even though `/ideas` (global idea search, distinct from a board's own view) is a required route — see the "Open gap" note in `SPEC/20-feature-client-ui.md` NAVIGATION.
